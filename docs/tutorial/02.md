...
When you run this example, if you didn't run `starplot setup` when [installing Starplot](../installation.md) then you should see console messages about downloading constellation and star [catalogs](../data/overview.md). Starplot does not include these catalogs in the PyPI package (to save space, and avoid PyPI's size limits), but once they're downloaded Starplot will just use the existing files. You can control where these files are downloaded [via the settings](../reference-settings.md).
