...
<figure markdown="span">
  ![Tutorial - Basic Star Chart](../images/tutorial/tutorial_02.svg){ width="600" }
</figure>
...
So, on line 19 we start by plotting the constellation lines. We plot these _first_, because Starplot can use the constellation lines to automatically find good places to plot object labels.
...
Once we're done plotting, we export the plot to an SVG file. Starplot also supports exporting to PNG, if you have the [Cairo graphics library](https://www.cairographics.org/) installed.

When you run this example, if you didn't run `starplot setup` when [installing Starplot](../installation.md) then you should see console messages about downloading constellation and star [catalogs](../data/index.md). Starplot does not include these catalogs in the PyPI package (to save space and avoid PyPI's size limits), but once they're downloaded Starplot will just use the existing files. You can control where these files are downloaded [via the settings](../reference-settings.md).
