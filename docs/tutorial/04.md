...
- `projection`: Instance of a Projection, which is the method to use for transforming the curved shape of the three-dimensional sky to a flat two-dimensional map. In this example, we use a `Mercator` projection, but Starplot supports many [types of projections](../reference-projections.md) and you can customize their properties (e.g. central RA/DEC).
