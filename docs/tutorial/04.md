...
<figure markdown="span">
  ![Tutorial - Map Plot](../images/tutorial/tutorial_04.svg)
</figure>
...
To create a general map, you use the [`MapPlot` class][starplot.MapPlot], which works similar to the zenith plot but takes a few different parameters:

- `projection`: Instance of a [Projection](../reference-projections.md), which is the method to use for transforming the curved shape of the three-dimensional sky to a flat two-dimensional map. In this example, we use a `Mercator` projection, but Starplot supports many [types of projections](../reference-projections.md) and you can customize their properties (e.g. central RA/DEC).
