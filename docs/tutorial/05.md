...
On line 22, we use a powerful feature of Starplot: looking up an object and then using that object's properties to help us create the plot. In this example, we look up The Pleiades open cluster by calling the `get` function on the `DSO` model. This function returns an instance of the [DSO model](../reference-models.md#starplot.DSO), which has various properties of the object, including it's position (RA/Dec).
...
- `optic`: An instance of an [optic](../reference-opticplot.md#starplot.optics.Optic). This example uses binoculars, but Starplot also supports refractor/reflector telescopes, generic scopes, and cameras.
