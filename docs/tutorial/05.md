...
<figure markdown="span">
  ![Tutorial - Map Plot](../images/tutorial/tutorial_05.svg){ width="600" }
</figure>

Starplot also has [optic plots][starplot.OpticPlot], which simulate what you'll see through an optic (e.g. binoculars, telescope, camera) at a specific time and place. The simulated view will show you the true field of view for the optic, and it will even orient the stars based on the location you specify and the most logical position of your optic.

Optic plots work very similar to map plots, with a few key differences: they also require an optic when you create them, and use a single coordinate as a target instead of a rectangular extent (since the extent will be determined by the field of view of the optic).
...
On line 22, we use a powerful feature of Starplot: looking up an object and then using that object's properties to help us create the plot. In this example, we look up The Pleiades open cluster by calling the `get` function on the `DSO` model. This function returns an instance of the [DSO model](../models/dso.md), which has various properties of the object, including its position (RA/Dec).
...
- `optic`: An instance of an [optic](../models/optics.md). This example uses binoculars, but Starplot also supports refractor/reflector telescopes, generic scopes, and cameras.
